...
- 审计分页列表页（`/page/2`、`?page=2`）：自引用 canonical、可抓取翻页链接、避免误 noindex 导致深层详情页成为孤儿页
...
- **分页列表页**：`/page/2`、`?page=2` 等 URL 会被正常收录；收录 ≠ 与第一页抢同一主词排名。现代做法是每页自引用 canonical、动态 Title 加页码后缀、用标准 `<a href>` 翻页链接，让蜘蛛沿分页发现深层详情页，而不是对 page 2+  blanket `noindex` 或全部 canonical 指回第一页

详见 `references/pagination-seo-principles.md`。
...
- [`references/pagination-seo-principles.md`](references/pagination-seo-principles.md) — 分页收录与 canonical：被索引 ≠ 与第一页自相残杀；自引用 canonical、可抓取翻页、避免深层孤儿页
